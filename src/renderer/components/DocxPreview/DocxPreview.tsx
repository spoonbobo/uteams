...
import { Box, Paper, Typography, Popover, Divider } from '@mui/material';
import { DocxPreviewProps, DocxPreviewRef, ElementHighlight } from './types';
...
const DocxPreview = React.forwardRef<DocxPreviewRef, DocxPreviewProps>(
  (
    {
      content,
      showStats = true,
    ...
    const handleTestHighlights = () => {
      ...
      const { elementCounts } = content;
      if (!elementCounts) {
        // eslint-disable-next-line no-console
        console.warn(
          '[DocxPreview] No element counts available for test highlights',
        );
        return;
      }
      ...
      if (availableElements.length === 0) {
        // eslint-disable-next-line no-console
        console.warn(
          '[DocxPreview] No valid elements found for test highlights',
        );
        return;
      }
      ...
      const maxHighlights = Math.min(
        4,
        Math.floor(availableElements.reduce((sum, e) => sum + e.count, 0) / 2),
      );
      ...
      for (let i = 0; i < numHighlights; i += 1) {
        ...
        const comment =
          comments[color][Math.floor(Math.random() * comments[color].length)];

        // Avoid duplicate highlights on same element
        const exists = testData.some(
          (h) =>
            h.elementType === elementType && h.elementIndex === elementIndex,
        );
        if (!exists) {
    ...
    React.useImperativeHandle(
      ...
      [addHighlight, clearHighlights, toggleAnimations],
    );
    ...
    useEffect(() => {
      ...
      const currentHighlightKeys = allHighlights.map(
        (h) => `${h.elementType}-${h.elementIndex}`,
      );
      const processedHighlightKeys = processedHighlights.map(
        (h) => `${h.elementType}-${h.elementIndex}`,
      );
      ...
      const hasChanges =
        currentHighlightKeys.length !== processedHighlightKeys.length ||
        !currentHighlightKeys.every((key) =>
          processedHighlightKeysSet.has(key),
        );
      ...
      const newHighlightKeys = currentHighlightKeys.filter(
        (key) => !processedHighlightKeys.includes(key),
      );
      ...
      if (newHighlightItems.length > 0) {
        // eslint-disable-next-line no-console
        console.log(
          `🎨 [DocxPreview] Found ${newHighlightItems.length} new highlights:`,
          newHighlightItems,
        );
    ...
    }, [allHighlights, processedHighlights]);
    ...
    return (
      <Box sx={sx}>
        ...
        {showHoverPreview && !showFullContent && (
          ...
          >
            <Paper
              sx={{ p: 2, maxWidth: 600, maxHeight: 500, overflow: 'auto' }}
            >
              <Typography
                variant="subtitle2"
                gutterBottom
