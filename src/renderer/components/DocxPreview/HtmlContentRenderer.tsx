...
function HtmlContentRenderer({
  ...
  useEffect(() => {
    ...
    if (
      ...
      newHighlightsToApply.length > 0
    ) {
      // eslint-disable-next-line no-console
      console.debug(
        `[HtmlContentRenderer] Applying ${newHighlightsToApply.length} new highlights`,
      );
    }

    // Apply each new highlight directly to the DOM
    newHighlightsToApply.forEach((highlight) => {
      ...
      if (targetElement) {
        ...
        const isNewHighlight = newHighlights.some(
          (newH) =>
            newH.elementType === elementType &&
            newH.elementIndex === elementIndex,
        );

        if (animateHighlights && isNewHighlight) {
          baseClasses.push('animate');
          // Add staggered delay for multiple new highlights
          const newHighlightIndex = newHighlights.findIndex(
            (newH) =>
              newH.elementType === elementType &&
              newH.elementIndex === elementIndex,
          );
        ...
        const existingClasses = targetElement.className
          .split(' ')
          .filter((cls) => !cls.startsWith('docx-highlight'));
        ...
        if (process.env.NODE_ENV === 'development') {
          // eslint-disable-next-line no-console
          console.debug(
            `[HtmlContentRenderer] Applied highlight to ${elementType} #${elementIndex}`,
          );
        }
      } else {
        ...
        if (!appliedHighlightsRef.current.has(`missing-${missingKey}`)) {
          ...
          if (process.env.NODE_ENV === 'development') {
            const availableElements = container.querySelectorAll(
              `[data-element-type="${elementType}"]`,
            );
            const availableIndices = Array.from(availableElements)
              .map((el) => el.getAttribute('data-element-index'))
              .filter(Boolean);
            // eslint-disable-next-line no-console
            console.debug(
              `[HtmlContentRenderer] Element not found: ${elementType} #${elementIndex}. Available: [${availableIndices.join(', ')}]`,
            );
          }
        }
    ...
    if (
      animateHighlights &&
      newHighlightsToApply.some((h) =>
        newHighlights.some(
          (newH) =>
            newH.elementType === h.elementType &&
            newH.elementIndex === h.elementIndex,
        ),
      )
    ) {
  ...
  useEffect(() => {
    ...
    const currentKeys = new Set(
      highlights.map((h) => `${h.elementType}-${h.elementIndex}`),
    );
    const appliedKeys = appliedHighlightsRef.current;

    // Find highlights that were removed
    const removedKeys = Array.from(appliedKeys).filter(
    ...
    if (removedKeys.length > 0) {
      ...
      if (process.env.NODE_ENV === 'development') {
        // eslint-disable-next-line no-console
        console.debug(
          `[HtmlContentRenderer] Removing ${removedKeys.length} highlights`,
        );
      }
